...
namespace ConsoleApp1
{
    ...
    {
        public Dictionary<string, Part3D> Parts = new();
        ...
        public void Draw(Shader shader, Matrix4 model, Matrix4 view, Matrix4 projection, double time)
        {
            if (visible)
            {
                foreach (Part3D part in Parts.Values)
                {
                    part.Draw(shader, roll * pitch * yaw * Matrix4.CreateTranslation(offset_x, offset_y, offset_z) * model, view, projection, time);
                }
            }
        }
